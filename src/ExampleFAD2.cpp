...
template <typename T> F<T> func(const F<T> *x_in, int n);
template <typename T> void show_result(F<T> &f_result, int n);
template <typename T, typename U>
void show_norms(F<T> &f_result1, F<U> &f_result2, int n);
int main()
{
  // double type
  // variables initiation
  F<double> f_double;                // Declare variables f
  F<double> x_double[ TERMS ];       //  Declare 2 variables
  x_double[ 0 ] = 0.512;             // Initialize variable x
  x_double[ 1 ] = 2.141;             // Initialize variable y
  x_double[ 0 ].diff(0, TERMS);      // Differentiate with respect to x (index 0 of 2)
  x_double[ 1 ].diff(1, TERMS);      // Differentiate with respect to y (index 1 of 2)
  ...
  // Stack-form template
  // variables initiation
  F<mpreal> f_mpreal;                // Declare variables x,y,f
  F<mpreal> x_mpreal[ TERMS ];       // Declare two variables
  x_mpreal[ 0 ] = 0.512;             // Initialize variable x
  x_mpreal[ 1 ] = 2.141;             // Initialize variable y
  x_mpreal[ 0 ].diff(0, TERMS);      // Differentiate with respect to x (index 0 of 2)
  x_mpreal[ 1 ].diff(1, TERMS);      // Differentiate with respect to y (index 1 of 2)
...
template <typename T>
void show_result(F<T> &f_result, int n)
{
